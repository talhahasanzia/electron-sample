...
import electron, { app as app$1, BrowserWindow, ipcMain as ipcMain$1, shell as shell$1, Menu } from "electron";
...
function createWindow() {
  ...
  win.webContents.on("did-finish-load", () => {
    ...
    try {
      win == null ? void 0 : win.setTitle("Entrifi");
      win == null ? void 0 : win.webContents.executeJavaScript("document.title = 'Entrifi'", true).catch(() => {
      });
    } catch (e) {
    }
  });
...
ipcMain$1.handle("save-submission", async (_event, submission) => {
  try {
    ...
    console.error("Error saving submission:", error2);
    return { success: false, error: String(error2) };
  }
});
ipcMain$1.handle("get-submissions", async () => {
  try {
    const submissions = store.get("submissions", []);
    return { success: true, data: submissions };
  } catch (error2) {
    console.error("Error getting submissions:", error2);
    return { success: false, error: String(error2) };
  }
});
ipcMain$1.handle("clear-submissions", async () => {
  try {
    store.set("submissions", []);
    return { success: true };
  } catch (error2) {
    console.error("Error clearing submissions:", error2);
    return { success: false, error: String(error2) };
  }
});
ipcMain$1.handle("print-to-pdf", async (_event) => {
  try {
    if (!win) {
    ...
    const pdfPath = path.join(os2.tmpdir(), `form-${timestamp}.pdf`);
    await fs2.writeFile(pdfPath, pdfData);
    await shell$1.openPath(pdfPath);
...
app$1.whenReady().then(() => {
  ...
  const menuTemplate = [
    ...
    ...isMac ? [
      {
        label: app$1.name || "Entrifi",
        submenu: [
          { role: "about", label: `About ${app$1.name || "Entrifi"}` },
          { type: "separator" },
          { role: "services" },
          { type: "separator" },
          { role: "hide" },
          ...
          { role: "unhide" },
          { type: "separator" },
          { role: "quit", label: `Quit ${app$1.name || "Entrifi"}` }
        ]
      }
    ] : [],
    {
      ...
      submenu: [
        {
          label: "Contact us",
          click: async () => {
            await shell$1.openExternal("https://github.com/talhahasanzia");
          }
        },
        {
          label: "Quit",
